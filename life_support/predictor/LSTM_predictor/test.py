...
def makePrediction(df):
    ...
    updatedPrediction['LSTM_mood_prediction'] = inv_yhat[0]
    ...
    #         values = values.astype('float32')
    #         scaler = MinMaxScaler(feature_range=(0,1))
    #         test_X = scaler.fit_transform(values)
...
def writeToJSON(currentIndexPredictions):
    with open('LSTM_predictor.json', 'r') as f:
        brackets = json.load(f)
    with open('LSTM_predictor.json', 'w') as f:
